...
import com.caipiao.quantanliu.caipiao.App;
import com.caipiao.quantanliu.caipiao.Zhibo;
...
import com.caipiao.quantanliu.caipiao.crawler.bean.ServerInfo;
import com.caipiao.quantanliu.caipiao.crawler.handler.MessageHandler;
import com.caipiao.quantanliu.caipiao.crawler.handler.ResponseParser;
import com.caipiao.quantanliu.caipiao.crawler.util.HttpUtil;
import com.caipiao.quantanliu.caipiao.crawler.util.LogUtil;
import com.caipiao.quantanliu.caipiao.crawler.util.MD5Util;
import com.caipiao.quantanliu.caipiao.crawler.util.SttCode;
import com.caipiao.quantanliu.caipiao.crawler.util.TimeHelper;
    ...
    private MessageHandler.OnReceiveListener danmakuListener = new MessageHandler.OnReceiveListener() {
        ...
        public void onReceive(List<String> responses) {
            ...
            for (String response : responses) {
                ...
                //双色球弹幕抓取匹配
                //-19-18-17-11-16-14-12-14-
                if (danmaku.getContent().matches(".*-[0-9][0-9]-[0-9][0-9]-[0-9][0-9]-[0-9][0-9]-[0-9][0-9]-[0-9][0-9]-[0-9][0-9]-.*")) {
                    DanmakuDao danmakuDao = App.getApp().getDaoSession().getDanmakuDao();
                    danmakuDao.insert(danmaku);
                    Zhibo.getInstance().currentDanmaKuList.add(danmaku);
                }
                if (danmakus.size() >= 20) {
